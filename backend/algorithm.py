...
def calc_probability_in_HZ(OrbSM, OrbSMi, HZ_Inner, HZ_Outer):
    ...
    s = 0.3 
    return (1 / (1.33)) * (1 / (s * math.sqrt(2 * math.pi))) * np.exp(-((x - 0.12) ** 2) / (2 * s ** 2))
...
def calc_probability_in_planet_size(planetSize):
    ...
    return (1 / (1.33)) * (1 / (s * math.sqrt(2 * math.pi))) * np.exp(-((x - 0.28) ** 2) / (2 * s ** 2))
...
def calc_habitability_prob(magnitude, spectral_type, distance, OrbSM, planetSize, planetEccentricity, HZ_weight, size_weight, eccentricity_weight):
    ...
    Planet_Eccentricity_Prob = calc_probability_in_planet_eccentricity(planetEccentricity)
    return (HZ_Prob * HZ_weight + Planet_Size_Prob * size_weight + Planet_Eccentricity_Prob * eccentricity_weight) / (HZ_weight+size_weight+eccentricity_weight) * 100